...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'create') {
      ...
      if (!cats.length) return res.status(400).json({ error: 'Elige al menos un tema' });
    ...
    if (action === 'delete') {
      const id = parseInt(body.id, 10);
      if (!id) return res.status(400).json({ error: 'Falta id' });
      const roleRow = await sql`SELECT role FROM people WHERE name = ${name}`;
      if (roleRow.rows[0]?.role !== 'admin') return res.status(403).json({ error: 'Solo un admin puede borrar entradas.' });
